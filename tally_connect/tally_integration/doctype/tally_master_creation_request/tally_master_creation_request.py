...
# 			self.assigned_to = "Administrator"# ← ADD THIS (simple fix)
...
# 			"Has Role",
# 			filters={"role": "Tally Admin", "parenttype": "User"},
# 			fields=["parent"],
# 			limit=1
# 		)
# 		return admins[0].parent if admins else "Administrator"
...
class TallyMasterCreationRequest(Document):
    ...
    def before_insert(self):
        ...
        # Auto-assign to Tally Admin
        # NOTE: simplified to avoid missing workflows.assignment import
        if not self.assigned_to:
            # self.assigned_to = self.get_next_available_admin()  # OLD (uses workflows)
            self.assigned_to = "Administrator"  # TODO: replace with real round-robin if needed
    ...
    def after_insert(self):
        ...
        """
        # Send notification to assigned admin
        # NOTE: temporarily disabled to avoid missing workflows.notification import
        # self.notify_assigned_admin()  # TODO: implement email later
    ...
    def validate(self):
        ...
        if self.erpnext_doctype and self.erpnext_document:
            if not frappe.db.exists(self.erpnext_doctype, self.erpnext_document):
                frappe.throw(
                    _("ERPNext {0} '{1}' does not exist").format(
                        self.erpnext_doctype, self.erpnext_document
                    )
                )
    ...
    def validate_status_transition(self, old_status, new_status):
        ...
        if new_status not in allowed:
            frappe.throw(
                _("Invalid status transition from '{0}' to '{1}'. Allowed: {2}").format(
                    old_status, new_status, ", ".join(allowed)
                )
            )
    ...
    def on_approve(self):
        ...
        # Notify requester
        # NOTE: temporarily disabled to avoid missing workflows.notification import
        # self.notify_requester_approval()  # TODO: implement email later

        # Log notification
        # NOTE: keep history, this does not depend on workflows
        self.add_notification_entry("approved", self.requested_by)
    ...
    def on_reject(self):
        ...
        # Notify requester
        # NOTE: temporarily disabled to avoid missing workflows.notification import
        # self.notify_requester_rejection()  # TODO: implement email later
    ...
    def on_complete(self):
        ...
        # Notify requester
        # NOTE: temporarily disabled to avoid missing workflows.notification import
        # self.notify_requester_completion()  # TODO: implement email later
    ...
    def on_failure(self):
        ...
        # Notify assigned admin about failure
        # NOTE: temporarily disabled to avoid missing workflows.notification import
        # self.notify_admin_failure()  # TODO: implement email later
        pass
    ...
    def get_suggested_master_name(self):
        ...
        if self.erpnext_doctype == "Customer":
            ...
            # Use code for items (more stable)
            name = doc.item_code
        else:
            name = doc.name
    ...
    def get_next_available_admin(self):
        """
        Get next available Tally Admin.

        NOTE: simplified implementation to remove dependency on
        tally_connect.tally_integration.workflows.assignment.
        """
        # Simple implementation: pick any user with "Tally Admin" role, else Administrator
        ...
        return has_role_rows[0].parent if has_role_rows else "Administrator"
    ...
    def retry_linked_transaction(self):
        ...
        from tally_connect.tally_integration.api.approval import (
            retry_linked_transaction_sync,
        )
    ...
    def notify_assigned_admin(self):
        ...
        NOTE: placeholder; original implementation depended on
        tally_connect.tally_integration.workflows.notification.
        """
        # Example simple email implementation (optional, can be enabled later):
        # frappe.sendmail(
        #     recipients=[self.assigned_to],
        #     subject=f"New Tally Master Request: {self.master_name}",
        #     message=f"A new Tally master creation request ({self.name}) has been assigned to you."
        # )
        pass
    ...
    def notify_requester_approval(self):
        ...
        Send approval notification to requester
        """
        # TODO: implement simple email if needed
        pass
...
def get_request_details(request_name):
    ...
    if request.erpnext_doctype and request.erpnext_document:
        try:
            _ = frappe.get_doc(request.erpnext_doctype, request.erpnext_document)
            current_data = request.capture_erpnext_data()
        except Exception:
            pass
...
def get_my_pending_requests():
    ...
    # Add requester names
    for req in requests:
        req["requester_name"] = frappe.db.get_value(
            "User", req["requested_by"], "full_name"
        )
